...
from config import AUTOWARE_DEFAULT_CONFIG_DIR_PATH
    ...
    def traverse_dict(dictionary, config_file_path, ruamel_yaml_config_file):
        for key, value in dictionary.items():
            if isinstance(value, dict):
                ...
                # print(f"Previous keys: {AutowareParser.key_stack}, Key: {key}, Value: {value}")
...
if __name__ == '__main__':
    ...
    tuned_id_list = [3, 6, 9]
    output_file_tuple_list = AutowareTranslator.option_obj_translator(config_file_obj.option_obj_list, tuned_id_list)
