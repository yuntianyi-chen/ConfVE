...
from apollo_v7_testing.auxiliary.map import map_tools
import pickle
from deap import base, creator, tools
...
def output_results():
    ...
    # dest="/apollo/apollo_v7_testing/scenario_generator"
...
def scenario_runner():
    ...
    for obs_group_folder_name in obs_group_folders_name_list:
        ...
        lanes,min_distance,speeding_min,uslc_min,fastAccl_min,hardBrake_min,sim_time,orcle_time,num_runs=runScenario(deme, record_name, obs_group_number=scenario_counter) 
        # lanes.remove('')
...
def runScenario(deme, record_name, obs_group_number):
    ...
    while failed:
        ...
        # scenario_player_cmd='bazel run //apollo_v7_testing/scenario_player:run_automation -- -rv 586980.86,4140959.45,587283.52,4140882.30'+' -o '+record_name+' -mn '+map_name+' -ogn '+str(obs_group_number)
